...
export class ChartController {
	...
	set values(values: number[]) {
		...
		for (const idx in values) {
			...
			const y =
				this.#size.height -
				this.#padding -
				((values[i] - this.#domain.min) / (this.#domain.max - this.#domain.min)) *
					(this.#size.height - 2 * this.#padding);
	...
	public drawLines() {
		for (let i = 0; i < this.#points.length - 1; i++) this.drawLine(this.#points[i], this.#points[i + 1]);
	}

	public drawPoints() {
		for (const point of this.#points) this.drawPoint(point);
	}
