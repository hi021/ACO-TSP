...
export class Ant {
	...
	public pickAndVisitNode(
		...
		const probabilityMatrix = this.calculateProbabilityMatrix(
			sourceId,
			nodes,
			pheromoneMatrix,
			inverseDistanceMatrix,
			parameters
		);
	...
	private calculateProbabilityMatrix(
		sourceId: number,
		nodes: Map<string, GraphNode>,
		pheromoneMatrix: number[][],
		inverseDistanceMatrix: number[][],
		...
		for (let i = 0; i < probabilityMatrix.length; ++i) {
			...
			const probability =
				desirability ** parameters.distancePriority * pheromoneIntensity ** parameters.pheromoneImportance;

			probabilityMatrix[i] = probability;
			probabilitySum += probability;
		}

		for (let i = 0; i < probabilityMatrix.length; ++i) probabilityMatrix[i] /= probabilitySum;
